...
class AllUsersListPage extends StatefulWidget {
  @override
  _AllUsersListPageState createState() => _AllUsersListPageState();
}
  ...
  Widget build(BuildContext context) {
    return EasyRefresh.custom(
      ...
      onRefresh: () async {
        ...
        return context.read<ContactsViewModel>().refreshAllUsersData();
      },
      onLoad: () async {
        _refreshController.finishRefresh();
        return context.read<ContactsViewModel>().loadAllUsersMoreData();
      },
      firstRefresh: true,
  ...
  Widget _buildSilverList(BuildContext context) {
      return Selector<ContactsViewModel, List<User>>(
        selector: (ctx, contactsVM) {
          return contactsVM.allUsersList;
        },
        shouldRebuild: (prev, next) => context.read<ContactsViewModel>().shouldRefreshAllUsers,
        //!ListEquality().equals(prev.feedsList, next.feedsList),
        builder: (ctx, feedList, child) {
          context.read<ContactsViewModel>().rebuildedAllUsers();
          return SliverList(
            delegate: SliverChildBuilderDelegate((context, index) {
              return Selector<ContactsViewModel, User>(
                  selector: (ctx, contactsVM) => contactsVM.allUsersList[index],
                  shouldRebuild: (prev, next) => true,
                  builder: (ctx, user, child) {
                    return Container(
                      child: ContactListItemView(user: user,onTap: (){NavigatorExt.pushToPage(context,CIRPersonalPage(user: user));},),
                    );
                  }
              );
            },
              childCount: context.read<ContactsViewModel>().allUsersTotal,
            ),
          );
        },
      );
